...
import com.example.medpro.databinding.ActivityEyeMedicineBinding

class EyeMedicineActivity : AppCompatActivity() {
    private lateinit var binding: ActivityEyeMedicineBinding
    private lateinit var labArrayList: ArrayList<MediModel>
    lateinit var back: Button
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_eye_medicine)


        getSupportActionBar()?.hide()
        binding = ActivityEyeMedicineBinding.inflate(layoutInflater)
        setContentView(binding.root)
        back = findViewById<Button>(R.id.button)

        val mediname = arrayOf(
            "Acetylcysteine drops", "Atropine"
        )
        val description = arrayOf(
            "drops used for fry eyes","used to relax the muscles in the eyes"
        )
