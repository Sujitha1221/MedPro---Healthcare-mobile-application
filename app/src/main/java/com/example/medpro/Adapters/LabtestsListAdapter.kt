...
import com.example.medpro.AllLabTests
import com.example.medpro.R
import com.example.medpro.Models.Lab

class LabtestsListAdapter(

    private val context: Context,
    private var resultTransaction: List<Lab>,
    private val activity: AllLabTests

) : RecyclerView.Adapter<LabtestsListAdapter.ViewHolderTransaction>() {
        ...
        return when (viewType) {
            VIEW_TYPE_DATA -> {//inflates row layout
                val view = LayoutInflater.from(parent?.context)
                    .inflate(R.layout.labtest_list, parent, false)
